...
function ProductStandFolding({ sectionRef }) {

  const router = useRouter();
  const { id } = router.query;

  const contentTableClass = router.asPath === "/boxes-campaign/folding-carton-a"

  return (
    <div className="product_stand_fold_margin" ref={sectionRef}>
      <Container>
        <div className={` ${contentTableClass ? "main_product_fold" : "main_product_stand"} `}>
          ...
          <div className={`${contentTableClass ? "content_section_fold" : "content_section"}`}>
...
export default ProductStandFolding;